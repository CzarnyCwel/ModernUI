...
function PANEL:Init()
	...
	self.moneyLabel.Paint = function(panel, width, height)
		surface.SetDrawColor(Color(17, 17, 17, 150))
...
function PANEL:Init()
	...
	self.storageInventory.Paint = function(panel, width, height)
		surface.SetDrawColor(Color(17, 17, 17, 55))
		surface.DrawRect(0, 0, width, height)
	end
	...
	ix.gui.inv1.Paint = function(panel, width, height)
		surface.SetDrawColor(Color(17, 17, 17, 55))
		surface.DrawRect(0, 0, width, height)
	end
...
function PANEL:SetLocalInventory(inventory)
	if (IsValid(ix.gui.inv1) and !IsValid(ix.gui.menu)) then
		...
		ix.gui.inv1:SetPos(self:GetWide() / 1.75 + self:GetFrameMargin() / 2, self:GetTall() / 2 - ix.gui.inv1:GetTall() / 2)
	end
end
...
function PANEL:SetStorageInventory(inventory)
	...
	self.storageInventory:SetPos(
		self:GetWide() / 2.25 - self.storageInventory:GetWide() - 2,
...
function PANEL:Paint(width, height)
	...
	surface.SetDrawColor(Color(255, 255, 255, 58))
	surface.DrawOutlinedRect( width * 0.5,0, 1, height, 1 )

	draw.DrawText(L"storage", "ixSubTitleFont",  width * 0.25, TEXT_ALIGN_CENTER, ix.config.Get("color"), TEXT_ALIGN_CENTER)
	draw.DrawText(L"yourInventory", "ixSubTitleFont",  width * 0.75, TEXT_ALIGN_CENTER, ix.config.Get("color"), TEXT_ALIGN_CENTER)
