...
function PANEL:Init()
	...
	--self.factionButtonsPanel.Paint = function(panel, width, height)
	...
	proceed:SetFont("ixMenuButtonFontSmall")
	...
	proceed:DockMargin(175, 2.5, 175, 2.5)
	proceed.Paint = function(panel, width, height)
	...
	factionBack:SetFont("ixMenuButtonFontSmall")
	...
	factionBack:DockMargin(175, 30, 175, 2.5)
	factionBack.Paint = function(panel, width, height)
		surface.SetDrawColor(0, 0, 0, 135)
		surface.DrawRect(0, 0, width, height)
	...
	self.attributesPanel:DockMargin(0, 20, 0, 20)
	...
	if (!self.bInitialPopulate) then
		...
		for _, v in SortedPairs(ix.faction.teams) do
			if (ix.faction.HasWhitelist(v.index)) then
				...
				button:DockMargin(175, 2.5, 175, 2.5)
...
function PANEL:Paint(width, height)
	...
	draw.DrawText(L("choose".. PageName), "ixSubTitleFont", 30, 2, ix.config.Get("color"), TEXT_ALIGN_LEFT)
end
